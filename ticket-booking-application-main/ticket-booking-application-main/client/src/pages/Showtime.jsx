...
const Showtime = () => {
    ...
    const fetchShowtime = async () => {
        try {
            ...
            console.error(error)
            toast.error(error.response?.data?.message || 'Error loading showtime', {
                position: 'top-center',
                autoClose: 2000
            })
        }
    ...
    const filteredSeats = showtime?.seats?.filter((seat) => (
        ...
        (!filterColumn || filterColumn.map(col => col.value).includes(String(seat.number)))
    ))
                        ...
                        <div className="flex flex-col justify-between rounded-b-lg bg-gradient-to-br from-indigo-100 to-white text-center text-lg drop-shadow-lg md:flex-row">
                            ...
                            {!!selectedSeats.length && (
                                <div className="flex flex-col items-center px-4 py-2">
                                    <p>Price per seat: ${showtime?.seatPrice}</p>
